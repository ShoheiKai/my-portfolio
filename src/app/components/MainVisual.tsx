...
function MainVisual(props: any) {
  ...
  scene.position.set(0, -2, 0); // x, y, zの位置を指定
  const [scale, setScale] = useState<Vector3>(new Vector3(0.7, 0.7, 0.7)); // 初期スケール
  ...
  const handleClick = () => {
    if (!clicked) {
      ...
      setTargetScale(new Vector3(0.8, 0.8, 0.8)); // サイズを変更
      ...
      setTimeout(() => {
        setTargetScale(new Vector3(0.7, 0.7, 0.7)); // 元のサイズに戻す
    ...
    >
      <Suspense fallback={null}>
        ...
        <Text3D
          ...
          rotation={[0, -0.2, -0.1]} // 右側が前に出るように傾ける
          font="/Shippori_Mincho_B1_ExtraBold_Regular.json" // フォントファイルへのパス
          height={0.2} // 厚みを追加
          size={1.2} // 文字のサイズ
        >
